...
public class SettingActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        backgroundPushing.setChecked(PreferenceManager.getDefaultSharedPreferences(this)
                ...
                if (isChecked) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                        if (!Settings.canDrawOverlays(SettingActivity.this)) {
                            new AlertDialog.Builder(SettingActivity.this).setTitle("后台上传视频")
                            ...
                            PreferenceManager.getDefaultSharedPreferences(SettingActivity.this)
                                    .edit().putBoolean(KEY_ENABLE_BACKGROUND_CAMERA, true).apply();
                        }
                    } else {
                        PreferenceManager.getDefaultSharedPreferences(SettingActivity.this)
                                .edit().putBoolean(KEY_ENABLE_BACKGROUND_CAMERA, true).apply();
                    ...
                    PreferenceManager.getDefaultSharedPreferences(SettingActivity.this)
                            .edit().putBoolean(KEY_ENABLE_BACKGROUND_CAMERA, false).apply();
                }
            }
        });
        ...
        x264enc.setChecked(PreferenceManager.getDefaultSharedPreferences(this).getBoolean(
                "key-sw-codec", false));
        ...
        x264enc.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                PreferenceManager.getDefaultSharedPreferences(SettingActivity.this).edit()
                        .putBoolean("key-sw-codec", isChecked).apply();
            }
        });

        CheckBox enable_video_overlay = (CheckBox) findViewById(R.id.enable_video_overlay);
        enable_video_overlay.setChecked(PreferenceManager.getDefaultSharedPreferences(this)
        ...
        enable_video_overlay.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                PreferenceManager.getDefaultSharedPreferences(SettingActivity.this)
                        .edit().putBoolean("key_enable_video_overlay", isChecked).apply();
            }
        });
    ...
    public void onScreenPushResolution(View view) {
        int defaultIdx = PreferenceManager.getDefaultSharedPreferences(this).getInt(
                "screen_pushing_res_index", 3);
        new AlertDialog.Builder(this).setTitle("推送屏幕分辨率").setSingleChoiceItems(
                new CharSequence[]{"1倍屏幕大小", "0.75倍屏幕大小", "0.5倍屏幕大小", "0.3倍屏幕大小",
                        "0.25倍屏幕大小", "0.2倍屏幕大小"}, defaultIdx, new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        PreferenceManager.getDefaultSharedPreferences(
                                SettingActivity.this).edit().putInt(
                                "screen_pushing_res_index", which).apply();
                        Toast.makeText(SettingActivity.this,
                                "配置更改将在下次启动屏幕推送时生效", Toast.LENGTH_SHORT).show();
                        dialog.dismiss();
                    }
                }).show();
    }
